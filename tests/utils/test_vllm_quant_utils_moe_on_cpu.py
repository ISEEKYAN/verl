...
def test_ds4_refit_uses_native_layerwise_lifecycle(monkeypatch):
    ...
    reload_module.initialize_layerwise_reload = lambda model: events.append(("initialize", model))
    reload_module.finalize_layerwise_processing = lambda model, model_config: events.append(
