...
def export_qat_weights(per_tensor_param, modules, qat_config, bridge):
    ...
    if bridge is None and not use_modelopt_fake_quant:
        model_config = QATWeightExporter._get_model_config(modules) if modules else None
        qat_weight_exporter = QATWeightExporter.from_hf_stream(qat_config, model_config=model_config)
    else:
