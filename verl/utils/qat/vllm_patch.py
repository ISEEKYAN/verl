...
def patched_current_nvfp4_dense_process_weights_after_loading(self, layer: torch.nn.Module) -> None:
    ...
    if group_size is None:
        raise ValueError(f"NVFP4 dense scheme group_size contract is missing for {type(self).__name__}")
    # vLLM's CompressedTensorsW4A4Fp4 declares this contract and uses it to
    # allocate weight_scale as [N, K / group_size]. Preserve that declaration
    # for the later checkpoint-layout restore instead of inferring it by shape.
    layer._modelopt_group_size = int(group_size)
