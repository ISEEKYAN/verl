    ...
    def update_weights_from_ipc(self, peft_config: dict = None, base_sync_done=False, use_shm: bool = False):
        ...
        if self._is_qat_model:
            ...
            quant_reload_states = [
                (
                    model,
                    prepare_quanted_weights_for_loading(model, self.model_runner.vllm_config),
                )
                for model in self._iter_all_models()
            ]
        else:
        ...
        if self._is_qat_model:
            ...
            for model, reload_state in quant_reload_states:
                process_quanted_weights_after_loading(model, reload_state, self.model_runner.vllm_config)
        else:
